...
const getDefaultSeedItems = (): Omit<MerchItem, "id" | "createdAt">[] => [
  {
...
export default function App() {
  ...
  const saveToLocalStorage = (newItems: MerchItem[]) => {
    localStorage.setItem("merch-keeper-items", JSON.stringify(newItems));
    setItems(newItems);
  };

  // Load items from API with graceful localStorage fallback
  const fetchItems = async () => {
    ...
    try {
      ...
      if (isGithub) {
        throw new Error("GitHub Pages environment detected. Switching to offline-safe local storage.");
      }
      ...
      console.warn("API load failed or GitHub Pages detected. Running in relative standalone localStorage mode:", err.message);
      setIsStaticFallback(true);
      
      const stored = localStorage.getItem("merch-keeper-items");
      if (stored) {
        try {
        ...
        // Run first time initializer from static list
        const defaults = getDefaultSeedItems();
        const seeded: MerchItem[] = defaults.map((item, idx) => ({
          ...item,
          id: `local_seed_${idx}_${Date.now()}`,
          createdAt: new Date().toISOString()
        }));
        localStorage.setItem("merch-keeper-items", JSON.stringify(seeded));
        setItems(seeded);
      }
    } finally {
      setIsLoading(false);
  ...
  const handleUpdateItem = async (id: string, updatedParams: Partial<MerchItem>) => {
    if (isStaticFallback) {
      const updatedList = items.map((item) =>
        item.id === id ? { ...item, ...updatedParams } : item
      );
      saveToLocalStorage(updatedList);
      setEditingItem(null);
      return;
